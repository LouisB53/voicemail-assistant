...
const [, , garageId, username, password, displayName] = process.argv;

if (!garageId || !username || !password) {
  console.error("Usage : node scripts/create-user.js <garage_id> <username> <password> [display_name]");
  process.exit(1);
}
...
try {
  db.prepare(`
    INSERT INTO users (garage_id, username, password_hash, display_name)
    VALUES (?, ?, ?, ?)
  `).run(garageId, username, hash, displayName || username);
